...
define(['jquery', 'eve', 'settings'], function ($, eve, settings) {
	var activation = function _visibiltyHandler () {
	...
	$(document).ready(function () {
		...
		eve.on(settings.appName + '.invisible.*.item', function () { visibilityHandler.invisible(eve.arguments[1]);});
		eve.on(settings.appName + '.activate.*.item', function () { visibilityHandler.show(eve.arguments[1]);});
		eve.on(settings.appName + '.deactivate.*.item', function () { visibilityHandler.hide(eve.arguments[1]);});
	});
